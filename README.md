...
فهرست کامل کلمات گیج‌کننده — خانواده گذاشتن/گزاردن/زاریدن، حیاط/حیات، حول/هول، حل/هل، خواستن/خاستن، تنوین‌ها و ترکیب‌های به/در/از — با فرق معنی در فایل **[WORDS.md](./WORDS.md)** است. کلمه‌ای جا افتاده؟ در Issues بگویید.
...
1. `fixes.js` دو فاز دارد: **حروف** (`ptfFixChars` — جایگزینی تک‌حرفی 1:1 مثل ی/ک عربی، همیشه امن) و **کلمات** (`ptfFixWords` — ادغام/حذف/فاصله‌گذاری). `ptfFixText` ترکیب هر دو روی کل متن است (خروج از فیلد/پیست/انتر).
2. حین تایپ (`ptfFixTypingValue`) متن قبل از کرسر به «بخش تمام‌شده + کلمه باز» تقسیم می‌شود (`ptfSplitStable`)؛ بخش تمام‌شده با قوانین کامل و کلمه باز فقط با قوانین تک‌کلمه‌ای امن (`ptfFixOpenWord` با گارد پیشوند استثنا، مثلا «مید» چون ممکن است «میدان» شود دست نمی‌خورد). کرسر دقیق از خروجی می‌آید، بدون حدس.
3. کلاس حروف `FA_L` فقط حروف واقعی است (بدون علائم `، ؛ ؟`) چون بازه `\u0600-\u06FF` نقطه‌گذاری را هم شامل می‌شود و مرز را می‌شکست؛ `\b` اسکی هم برای فارسی کار نمی‌کند.
4. `content.js` روی `input`/`keydown`/`paste`/`change`/`focusin`/`focusout` در document و همه shadowRootها گوش می‌دهد؛ مقدار با native setter ست می‌شود تا React/Vue برنگرداند. URL/ایمیل با placeholder محافظت می‌شود.
5. تنظیمات در `chrome.storage.sync` است و بین پاپ‌آپ/تنظیمات/تب‌ها زنده همگام می‌شود.
...
- **v1.8.0:** دایره لغات چندبرابر (۲۰۰+ ترکیب: تنوین‌ها، همزه‌ها، به/در/از، کم/پر/خوش/بد/نا، هر/هیچ/همین، روزهای هفته و...) + رفع ۸ باگ موتور از فاز تست (فازر ۳۰۰۰ نمونه‌ای به صفر رسید)
- **v1.7.1:** متن «پاسداشت زبان پارسی» به یاد فردوسی بزرگ — با سره‌ترین واژه‌های پارسی — در صفحه تنظیمات و پاپ‌آپ
- **v1.7.0:** پوشش همه‌فیلد — اجرای document_start (لیسنر زودتر از اسکریپت سایت)، keydown همه کلیدها + beforeinput/compositionend/cut/drop، عدم دست‌کاری حین composition (Gboard/IME)، تور ایمنی با drill داخل شدو
- **v1.6.0:** دایره کلمات دوحالته بزرگ شد (تنوین‌ها، همزه‌ها، به/در/از، حول/هول، حل/هل، خواستن/خاستن) + فایل [WORDS.md](./WORDS.md) + حفظ تنوین درست (لطفاً)
- **v1.5.0:** بازطراحی ضدخرابی — حین تایپ فقط «بخش تمام‌شده قبل از کرسر» با قوانین کامل فیکس می‌شود و کلمه باز فقط قوانین تک‌کلمه‌ای امن می‌گیرد (با گارد پیشوند استثنا)؛ کرسر دقیق بدون حدس؛ رفع ریشه‌ای باگ «بخوابیم اما» + کلاس حروف بدون علائم `، ؛ ؟`
- **v1.4.3:** تکمیل رفع باگ «بخوابیم اما» — قانون «خانه ی» هم در live ته متن را مرز نمی‌داند + باکس‌های تست پاپ‌آپ/تنظیمات هم مثل تایپ واقعی (live) شدند
- **v1.4.2:** رفع باگ «بخوابیم اما ← بخوابیمما» — حالت live حین تایپ: تهِ متنِ ناتمام مرز پسوند نیست (خروج از فیلد/پیست/انتر حالت کامل)
